...
function Teacher_Details() {
  ...
  const fetchStudentDetails = async () => {
    const response = await axios
      .get(
        `https://8nnc5jq04m.execute-api.us-east-1.amazonaws.com/music_portal/teachers/${urlParameters.token}/${urlParameters.teacher_id}`
        // {headers:{
              ...
              <div className="col-lg-9">
                <div className="card shadow-sm">
                  <div className="card-header bg-transparent border-0">
                    <h3 className="mb-0">
                      <i className="far fa-clone pr-1"></i>Instruments Teaching
                    </h3>
                  </div>
                  <div className="card-body pt-0">
                  <Lessons value={profile}/>
                  </div>
                </div>
              </div>
